...
Zenith runs continuous screening and management cycles, deploying capital into high-quality Meteora DLMM pools and closing positions based on live PnL, yield, and range data. It learns from every position it closes.

> Built on top of [Meridian](https://github.com/yunus-0x/meridian) — extended with risk scoring, circuit breaker, whale tracking, MEV protection, and multi-timeframe momentum.
...
Zenith runs a **ReAct agent loop** — each cycle the LLM reasons over live data, calls tools, and acts. Two specialized agents run on independent cron schedules:
...
Zenith loads envrypt-style encrypted values automatically. Keep `.env.raw` and `.envrypt` local; both are gitignored.
...
On startup Zenith fetches your wallet balance, open positions, and top pool candidates, then begins autonomous cycles immediately.
...
Install [Claude Code](https://claude.ai/code) and use it from inside the zenith directory. Claude Code has built-in agents and slash commands that use the `meridian` CLI under the hood.
...
```bash
zenith positions
zenith pnl <position_address>
zenith wallet-positions --wallet <addr>
```

**Screening**

```bash
zenith candidates --limit 5
zenith pool-detail --pool <addr> [--timeframe 5m]
zenith active-bin --pool <addr>
zenith search-pools --query <name_or_symbol>
zenith study --pool <addr> [--limit 4]
```

**Token research**

```bash
zenith token-info --query <mint_or_symbol>
zenith token-holders --mint <addr> [--limit 20]
zenith token-narrative --mint <addr>
```

**Deploy & manage**

```bash
zenith deploy --pool <addr> --amount <sol> [--bins-below 69] [--bins-above 0] [--strategy bid_ask|spot|curve] [--dry-run]
zenith claim --position <addr>
zenith close --position <addr> [--skip-swap] [--dry-run]
zenith swap --from <mint> --to <mint> --amount <n> [--dry-run]
zenith add-liquidity --position <addr> --pool <addr> [--amount-x <n>] [--amount-y <n>] [--strategy spot]
zenith withdraw-liquidity --position <addr> --pool <addr> [--bps 10000]
```

**Agent cycles**

```bash
zenith screen [--dry-run] [--silent]   # one AI screening cycle
zenith manage [--dry-run] [--silent]   # one AI management cycle
zenith start [--dry-run]               # start autonomous agent with cron jobs
```
...
```bash
zenith lessons
zenith lessons add "your lesson text"
zenith performance [--limit 200]
zenith evolve
zenith pool-memory --pool <addr>
```

**Blacklist**

```bash
zenith blacklist list
zenith blacklist add --mint <addr> --reason "reason"
```

**Discord signals**

```bash
zenith discord-signals
zenith discord-signals clear
```
...
Zenith sends notifications automatically for:
...
Zenith no longer auto-registers the first chat for safety. You must set:
...
HiveMind sync uses Agent Zenith at `https://api.agentmeridian.xyz` by default with the built-in public key. Agents can register, pull shared lessons/presets, and push learning events without a separate registration flow.

**What you get:**
- Shared lessons from other Zenith agents
...
HiveMind failures are non-blocking. If Agent Zenith is unavailable, the agent logs a warning and keeps running.

### Setup

No manual HiveMind registration command is required for the shared Agent Zenith setup. `agentId` is generated automatically on startup if it is missing.
...
Blank `hiveMindUrl` and `hiveMindApiKey` values intentionally fall back to the Agent Zenith defaults. Set `hiveMindPullMode` to `manual` if you do not want shared lessons and presets pulled automatically.

### Disable

There is currently no empty-string disable path for HiveMind; blank values fall back to the built-in Agent Zenith defaults. A true off switch should be implemented as an explicit config flag before documenting HiveMind as disabled by clearing fields.
...
hivemind.js         Agent Zenith HiveMind sync
