#!/usr/bin/env node
/**
 * zenith — Solana DLMM LP Agent CLI
...
// ─── Load .env from ~/.zenith/ if present ────────────────────────
const zenithDir = path.join(os.homedir(), ".zenith");
const zenithEnv = path.join(zenithDir, ".env");
if (fs.existsSync(zenithEnv)) {
  loadEnv({
    envPath: zenithEnv,
    keyPath: path.join(zenithDir, ".envrypt"),
    override: false,
  });
}
...
// ─── SKILL.md generation ──────────────────────────────────────────
const SKILL_MD = `# zenith — Solana DLMM LP Agent CLI
...
### zenithtoken-info --query <mint_or_symbol>
...
### zenithtoken-holders --mint <addr> [--limit 20]
