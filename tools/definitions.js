...
WARNING: This executes a real on-chain transaction. Check DRY_RUN mode.`,
      parameters: {
        ...
        properties: {
          ...
          risk_score: { type: "number", description: "Risk score 0-100 from screening (passed for tracking)" }
        },
        required: ["pool_address"]
      }
    }
...
Providers: llmProvider, screeningProvider, managementProvider, generalProvider, screeningBaseUrl, managementBaseUrl, generalBaseUrl, screeningApiKey, managementApiKey, generalApiKey
...
Reason is optional but helpful — logged as a lesson when provided.`,
...

  {
    ...
    function: {
      ...
      description: `Get the current circuit breaker status. Shows if new deploys are paused due to losses, drawdown, or manual trip.`,
...

  {
    ...
    function: {
      name: "reset_circuit",
      description: `Manually reset the circuit breaker (resume new deploys). Use when user explicitly says to resume or after market conditions normalize.`,
      parameters: {
        type: "object",
        properties: {
          reason: { type: "string", description: "Why resetting (logged for audit)" }
        }
      }
    }
  },

  {
    ...
    function: {
      name: "trip_circuit",
      description: `Manually trip the circuit breaker to pause new deploys. Use when market shows danger signals or user says to stop.`,
      parameters: {
        type: "object",
        properties: {
          reason: { type: "string", description: "Why tripping" },
          cooldown_hours: { type: "number", description: "Auto-reset after N hours. Omit for manual reset only." }
        },
        required: ["reason"]
      }
    }
...

  {
    ...
    function: {
      name: "check_honeypot",
      description: `Pre-deploy honeypot check for a token mint. Verifies mint/freeze authority, holder concentration, dev migrations. Returns safe/warnings/hard_fail.`,
      parameters: {
        type: "object",
        properties: {
          mint: { type: "string", description: "Token mint address" },
          symbol: { type: "string", description: "Token symbol (for logging)" }
        },
        required: ["mint"]
      }
    }
...

  {
    type: "function",
    function: {
      name: "check_whale_movements",
      description: `Check if top holders of a token have made significant moves since last snapshot.
Detects: whale exits, large dumps (>20% of holdings), new whale entries, concentration spikes.
Use during management to detect early rug signals — if a top10 holder dumps, consider closing.
First call creates a baseline snapshot; subsequent calls compare against it.`,
      parameters: {
        type: "object",
        properties: {
          mint: { type: "string", description: "Token mint address" },
          pool_name: { type: "string", description: "Pool name for logging" }
        },
        required: ["mint"]
      }
    }
...

  {
    ...
    function: {
      name: "get_mev_status",
      description: `Get current MEV risk level and recommended priority fee.
Shows: current MEV risk hour, recommended priority fee tier, estimated SOL cost.
Use before large deploys to decide timing and fee strategy.`,
      parameters: { type: "object", properties: {} }
    }
  },

  {
    ...
    function: {
      name: "check_momentum",
      description: `Multi-timeframe momentum check for a pool (5m + 30m + 1h).
Confirms entry by checking if volume, fees, and price trends are aligned across timeframes.
Returns: confirmed (bool), overall direction, per-timeframe breakdown, momentum score (-100 to +100).
BULLISH across 2+ timeframes = strong entry. BEARISH across 2+ = skip.
Use before deploying to avoid entering against the trend.`,
      parameters: {
        type: "object",
        properties: {
          pool_address: { type: "string", description: "Pool address to check" },
          pool_name: { type: "string", description: "Pool name for display" }
        },
        required: ["pool_address"]
      }
    }
...

  {
    ...
    function: {
      name: "list_providers",
      description: `List all available LLM provider presets with their base URLs and default models.
Use this when the user asks about available providers, wants to switch providers, or needs to know what options are available.
Supported providers: openrouter, anthropic, google, groq, together, deepseek, xai, local.`,
      parameters: { type: "object", properties: {} }
    }
  },
