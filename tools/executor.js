...
import { preDeployVerification } from "../honeypot-check.js";
function require_providers() { return { listProviders: _listProviders }; }
import fs from "fs";
...
function normalizeConfigValue(key, value) {
  const booleanKeys = new Set([
    ...
    "autoSwapAfterClaim",
    "trailingTakeProfit",
    "requireBullishSupertrend",
    "requireMomentumNotBearish",
    "solMode",
    "darwinEnabled",
    "lpAgentRelayEnabled",
    "ilVolatilityHaircut",
...
const toolMap = {
  ...
  list_providers: () => {
    const { listProviders } = require_providers();
    return { providers: listProviders() };
  },
  get_circuit_status: () => getCircuitStatus(),
  reset_circuit: ({ reason } = {}) => resetCircuit(reason || "Manual reset via tool"),
  trip_circuit: ({ reason, cooldown_hours } = {}) => tripCircuit({
    reason: reason || "Manual trip via tool",
    cooldownHours: cooldown_hours || 1,
    manual: cooldown_hours == null,
  }),
  check_honeypot: async ({ mint, symbol } = {}) => {
    const { checkHoneypotSignals } = await import("../honeypot-check.js");
    return checkHoneypotSignals({ mint, symbol });
  },
  check_whale_movements: async ({ mint, pool_name } = {}) => {
    const { checkWhaleMovements } = await import("../whale-tracker.js");
    return checkWhaleMovements(mint, pool_name);
  },
  get_mev_status: async () => {
    const { getMevStatus } = await import("../mev-protection.js");
    return getMevStatus();
  },
  check_momentum: async ({ pool_address, pool_name } = {}) => {
    const { checkMultiTimeframeMomentum } = await import("../multi-timeframe.js");
    return checkMultiTimeframeMomentum(pool_address, pool_name);
  },
  ...
  update_config: ({ changes, reason = "" }) => {
    ...
    const CONFIG_MAP = {
      ...
      requireBullishSupertrend: ["screening", "requireBullishSupertrend"],
      requireMomentumNotBearish: ["screening", "requireMomentumNotBearish"],
      supertrendTimeframe: ["screening", "supertrendTimeframe"],
      ...
      screeningProvider: ["llm", "screeningProvider"],
      managementProvider: ["llm", "managementProvider"],
      generalProvider: ["llm", "generalProvider"],
      screeningBaseUrl: ["llm", "screeningBaseUrl"],
      managementBaseUrl: ["llm", "managementBaseUrl"],
      generalBaseUrl: ["llm", "generalBaseUrl"],
      screeningApiKey: ["llm", "screeningApiKey"],
      managementApiKey: ["llm", "managementApiKey"],
      generalApiKey: ["llm", "generalApiKey"],
      // strategy
      ...
      deployMode: ["strategy", "deployMode"],
      // hivemind
  ...
  try {
    ...
    if (success) {
      if (name === "swap_token" && result.tx) {
        ...
        // Evaluate circuit breaker triggers post-close
        try {
          const { evaluateCircuitTriggers } = await import("../circuit-breaker.js");
          ...
          const [history, balances] = await Promise.all([
            getPerformanceHistory({ hours: 6, limit: 30 }),
            getWalletBalances({}),
          ]);
          evaluateCircuitTriggers(history?.positions || [], balances?.total_usd || 0);
        } catch (e) {
...
async function runSafetyChecks(name, args) {
  switch (name) {
    case "deploy_position": {
      ...
      const circuit = isCircuitTripped();
      if (circuit.tripped) {
        return {
          pass: false,
      ...
      if (args.base_mint) {
        try {
          const verification = await preDeployVerification({
            mint: args.base_mint,
            symbol: args.pool_name,
            riskScore: args.risk_score != null ? { score: args.risk_score } : null,
          });
          if (!verification.cleared) {
            return {
              pass: false,
              reason: `Honeypot/risk pre-check failed: ${verification.issues.join("; ")}`,
            };
          }
        } catch (error) {
          log("safety_warn", `Honeypot check error (allowing deploy): ${error.message}`);
        }
      }
