...
function scoreCandidate(pool) {
  ...
  let base = feeTvl * 1000 + organic * 10 + volume / 100 + holders / 100;

  // Volume trend bonus: rising volume = more fee potential
  const volChange = Number(pool.volume_change_pct || 0);
  if (volChange > 20) base *= 1.15;
  else if (volChange > 0) base *= 1.05;
  else if (volChange < -30) base *= 0.80;

  // Fee trend bonus: rising fees = proven fee generation
  const feeChange = Number(pool.fee_change_pct || 0);
  if (feeChange > 20) base *= 1.10;
  else if (feeChange < -30) base *= 0.85;

  // Smart money presence: strong conviction signal
  if (pool.smart_money_buy) base *= 1.20;
  if (pool.kol_in_clusters) base *= 1.10;

  // Penalties
  if (pool.is_pvp) base *= 0.60;
  if (pool.is_rugpull) base *= 0.30;
  if (pool.dex_boost || pool.dex_screener_paid) base *= 0.95;
...
export async function getTopCandidates({ limit = 10 } = {}) {
  ...
  // Attach risk score to each candidate (computed from full enrichment)
  for (const candidate of eligible) {
    try {
      const scoreObj = calculateRiskScore({ pool: candidate });
      candidate.risk_score = scoreObj.score;
      candidate.risk_tier = scoreObj.tier_name;
      candidate.size_multiplier = scoreObj.size_multiplier;
    } catch (error) {
      log("risk_score_warn", `Failed for ${candidate.name}: ${error.message}`);
    }
  }

  // Sort by risk score descending (best conviction first)
  eligible.sort((a, b) => (b.risk_score ?? 0) - (a.risk_score ?? 0));
