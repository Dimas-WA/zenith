...
export function evolveThresholds(perfData, config) {
  ...
  // ── 1. maxBinsBelow (volatility-based range) ──────────────────
  // If losers tend to have narrow ranges (low bins_below) → widen the minimum.
  // If winners cluster at certain volatility levels → adjust bins accordingly.
  {
    ...
    const current    = config.strategy.maxBinsBelow;

    if (loserVols.length >= 2 && winnerVols.length >= 1) {
      const avgLoserVol  = avg(loserVols);
      const avgWinnerVol = avg(winnerVols);
      if (avgLoserVol > avgWinnerVol * 1.3) {
        const target  = Math.max(current - 5, config.strategy.minBinsBelow);
        const newVal  = Math.round(clamp(nudge(current, target, MAX_CHANGE_PER_STEP), 35, 100));
        if (newVal !== current) {
          changes.maxBinsBelow = newVal;
          rationale.maxBinsBelow = `Losers had higher avg volatility ${avgLoserVol.toFixed(1)} vs winners ${avgWinnerVol.toFixed(1)} — adjusted maxBinsBelow from ${current} → ${newVal}`;
        }
      }
    }
  }
